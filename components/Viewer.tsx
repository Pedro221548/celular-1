  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        setError('O servidor de sinalização está offline.');
      }
    };
    ...
    <div className="min-h-screen p-4 md:p-8 flex flex-col items-center">
      <div className="w-full max-w-4xl bg-slate-800 rounded-2xl shadow-2xl overflow-hidden border border-slate-700">
        <div className="p-6 border-b border-slate-700 flex justify-between items-center bg-slate-900/50">
          <div>
            <h1 className="text-xl font-bold text-blue-400">Panel do Viewer</h1>
        ...
        <div className="aspect-video bg-black relative flex items-center justify-center">
          {status === ConnectionStatus.ERROR && (
            <div className="absolute inset-0 flex flex-col items-center justify-center z-10 bg-slate-900/95 p-8 text-center">
              ...
              <h2 className="text-xl font-bold text-white mb-2">Servidor Offline</h2>
              <p className="text-slate-400 mb-6 max-w-sm">Não foi possível conectar ao sistema de sinalização. Certifique-se de que o backend está rodando.</p>
              <div className="bg-slate-950 p-4 rounded-lg font-mono text-xs text-blue-400 border border-slate-800 text-left">
                <p className="mb-1 text-slate-500"># No terminal do projeto:</p>
                <p>node server.js</p>
              </div>
              <button onClick={() => window.location.reload()} className="mt-6 text-sm text-blue-500 hover:underline">Tentar reconectar</button>
            </div>
          )}
          ...
          {status === ConnectionStatus.CONNECTING && (
            <div className="animate-pulse flex flex-col items-center">
              ...
              <p className="text-slate-400">Estabelecendo conexão...</p>
            </div>
          )}
        ...
        <div className="p-6 bg-slate-900/30">
          <label className="block text-sm font-medium text-slate-400 mb-2">Compartilhe este link com o celular:</label>
